...
const SceneGraphViewer = dynamic(() => import('./components/SceneGraphViewer'), { ssr: false })

const DEMOS = [
  { id: 1, label: 'Security', icon: FaShieldAlt, image: '/security.jpg', objects: [{id:1,class:'person',confidence:0.64},{id:2,class:'person',confidence:0.28},{id:3,class:'person',confidence:0.26}], relationships:[{subject:'person',predicate:'near',object:'person'},{subject:'person',predicate:'near',object:'person'}] },
  { id: 2, label: 'Retail', icon: FaShoppingCart, image: '/crowd.jpg', objects: [{id:1,class:'person',confidence:0.88},{id:2,class:'person',confidence:0.68},{id:3,class:'person',confidence:0.57},{id:4,class:'person',confidence:0.53}], relationships:[{subject:'person',predicate:'near',object:'person'}] },
  { id: 3, label: 'Traffic', icon: FaCar, image: '/traffic.jpg', objects: [{id:1,class:'car',confidence:0.95}], relationships:[] },
  { id: 4, label: 'Crowd', icon: FaUsers, image: '/crowd.jpg', objects: [{id:1,class:'person',confidence:0.88},{id:2,class:'person',confidence:0.68},{id:3,class:'person',confidence:0.57},{id:4,class:'person',confidence:0.53},{id:5,class:'person',confidence:0.47}], relationships:[{subject:'person',predicate:'near',object:'person'}] },
]
          ...
          {DEMOS.map((demo) => (
            <button key={demo.id} onClick={() => runDemo(demo.id)}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all ${
                activeDemo === demo.id ? 'bg-cyan-500/20 border border-cyan-500 text-cyan-400' : 'bg-dark-800 border border-dark-700 hover:border-dark-600'
              }`}>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="space-y-4">
            <div className="aspect-video bg-dark-900 rounded-xl overflow-hidden border border-dark-700 relative">
              ...
              {showAnnotated && <div className="absolute bottom-2 left-2 bg-black/70 px-2 py-1 rounded text-xs text-cyan-400">🔍 YOLO26 Detection</div>}
            </div>
            ...
            <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder='Query: "find all people"'
            ...
            <button onClick={analyzeImage} disabled={!file || isAnalyzing}
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold flex items-center gap-2">
              <FaChartLine className="text-cyan-400" />Analysis Results
            </h3>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-dark-900 rounded-xl p-4 border border-dark-700">
                  ...
                  <div className="space-y-2 max-h-32 overflow-y-auto">
                    ...
                    {result.relationships.length === 0 && <p className="text-gray-500 text-sm">No relationships detected</p>}
                  </div>
                </div>
              </div>
            ) : (
